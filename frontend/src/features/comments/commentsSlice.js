...
export const fetchComments = createAsyncThunk("comments/fetchProjectComments", async (_, thunkAPI) => {
	try {
...
export const addComment = createAsyncThunk("comments/addComment", async ({ projectId, comment }, thunkAPI) => {
	try {
...
export const updateComment = createAsyncThunk("comments/updateComment", async (updatedCommentInfo, thunkAPI) => {
...
export const deleteComment = createAsyncThunk("comments/deleteComment", async ({ projectId, commentId }, thunkAPI) => {
	try {
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(addComment.fulfilled, (state, action) => {
				...
				state.status = "succeeded";
				state.commentsByProject[comment.projectId] = state.commentsByProject[comment.projectId] || [];
				state.commentsByProject[comment.projectId].push(comment);
			})
			...
			.addCase(updateComment.fulfilled, (state, action) => {
				...
				const index = state.allComments.findIndex((comment) => comment._id === updatedComment._id);
				if (index !== -1) {
					state.allComments[index] = updatedComment;
				}
				state.commentsByProject[updatedComment.projectId] = state.commentsByProject[updatedComment.projectId].map(
			...
			.addCase(deleteComment.fulfilled, (state, action) => {
				...
				if (state.commentsByProject[projectId]) {
					state.commentsByProject[projectId] = state.commentsByProject[projectId].filter(
...
export const selectCommentsByProject = (state) => state.comments.commentsByProject;
