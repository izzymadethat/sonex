...
const CommentsSideBar = ({ user, projectId }) => {
	...
	const comments = useSelector(
		(state) => state.comments.commentsByProject?.[projectId] || [],
	);
	...
	const emailMappings = JSON.parse(
		localStorage.getItem("emailMappings") || "{}",
	);
	...
	const isCommentOwner = (comment) => {
		return (
			...
			Object.keys(emailMappings).some(
				(email) => email === comment.email && emailMappings[email] === clientId,
			)
		);
	};
						...
						{comments.map((comment) => (
							<div key={comment.id} className="mt-4">
								<div className="flex items-center gap-4">
									<h2 className="underline text-lg/relaxed">{comment.email}</h2>
										...
										<div className="flex items-center gap-2">
											{comment.type === "revision" && user && (
												<Checkbox
													...
													onCheckedChange={() => {
														handleCommentComplete(comment);
													}}
												/>
											)}
											<p
												className={`max-w-xs leading-5 ${comment.isCompleted ? "line-through" : ""}`}
											>
												{comment.text}
											</p>
										</div>
