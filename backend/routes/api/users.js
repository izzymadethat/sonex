...
const validateClientInput = [
	...
	check("email").exists({ checkFalsy: true }).isEmail().withMessage("Please enter a valid email address"),
	handleValidationErrors,
];
...
const validateQuery = [
	query("page").exists({ checkFalsy: true }).isInt({ min: 1 }).withMessage("Page must be a positive integer").toInt(),
	query("size")
		.exists({ checkFalsy: true })
		.isInt({ min: 10, max: 75 })
